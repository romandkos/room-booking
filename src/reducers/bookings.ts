...
export const fetchBookingFromItems = (floorId: string) => (dispatch: any) => {
  return Promise.all([
    axios.get(
      `/v2/${getEndpoint("desk")}?floorId=${floorId}&includeCustomFields=true`
    ),
    axios.get(
      `/v2/${getEndpoint("room")}?floorId=${floorId}&includeCustomFields=true`
    ),
  ])
    .then(([assets, floors]) => {
      const bookings = [...assets.data.features, ...floors.data.features]
        .flatMap((feature: any) => {
          if (
            feature.properties.customFields &&
            feature.properties.customFields.bookings
          ) {
            return feature.properties.customFields.bookings.bookings.map(
              (booking: Booking) => {
                booking["itemId"] = feature.id;
                booking["date"] = moment(booking.date);
                booking["type"] =
                  feature.resourceType === "Space" ? "room" : "desk";
                return booking;
              }
            );
          }
...
export const updateBooking =
  (updatedBooking: Booking, bookings: Booking[]) => (dispatch: any) => {
    dispatch(startSaveBooking());
...
export const deleteBooking =
  (removeBooking: Booking, bookings: Booking[]) => (dispatch: any) => {
    dispatch(startSaveBooking());

    const newBookingsList: Booking[] = bookings.filter(
      (booking) => booking.key !== removeBooking.key
    );
    const newBookingsListForItem: Booking[] = newBookingsList.filter(
